...
- Alpha 上新 / 下架：每次 REST 刷新对比 BSC 列表。新合约立刻提醒并优先进入热池；消失或 offline 要连续缺失才报下架
...
`/api/health` 的 `memory` 给出 RSS、堆占用和是否处于内存承压；`hotPool.size` 是当前热池大小，`hotPool.max` 是当前上限。`listingWatch` 给出上次列表 diff、新上/下架计数和尚未确认的缺失数。健康检查脚本只判断 `"ok":true`，这些字段给运维看压力用。

## 热池与内存（Phase 2，仅免费数据）

即时路径只服务热池，避免 600 多个币每个 ticker 都重算。成员按这个顺序挤进上限：模拟仓、自动管理仓、近两小时新上架、近 2 小时提醒、高分观察、早期阶段。同一币占一个名额。新上架在热池里按关键成员处理，内存承压时仍会即时重算。
...
每次约 5 秒的 REST 刷新，都会用本轮 BSC Alpha 列表对比上一轮。主键是合约地址；没有合法合约时才用官方 `alphaId` 或 `tokenId`，不用显示名。数据仍是现有的免费 Binance Alpha 列表，不新增接口。

- 新出现的币马上进入提醒中心，类型 `alpha-new`（listing）。文案包含符号、合约、上线时间、现价和流动性，并在两小时内优先进入热池，方便 WebSocket 即时重算早期信号。
- 从列表消失，或被标成 `offline` 的币，默认要连续 3 次刷新都见不到才报 `alpha-delist`（delisting）。中途回来就清掉计数，避免接口抖一下就刷下架。
- 同一合约的同一类事件默认 6 小时内不重复提醒。单轮新上、下架都有条数上限。
- 空列表、列表突然缩掉一大截，或一次冒出很多「新币」时，本轮只跳过，不改快照，也不把整池报成下架或上新。
- 快照只保留当前列表，外加尚未确认的缺失项；条数有上限，不去为全市场再存一份价格历史。

`/api/health` 的 `listingWatch` 包含 `lastDiffAt`、`lastListings`、`lastDelistings`、`listings`、`delistings` 和 `pendingMissing`。

验证不用等真币上架，测试用内存里的假列表：

```bash
npm test
```

`test/listing-watch.test.js` 覆盖三件事：新合约会出上新提醒、缺一轮又回来不会下架、连续缺失达到确认次数才会下架。热池用例确认新上架会排在高分币前面。

| 变量 | 默认 | 作用 |
| --- | --- | --- |
| `ALPHAPULSE_LISTING_CONFIRM_MISSES` | 3 | 连续缺失几次才报下架，最少 2 |
| `ALPHAPULSE_LISTING_COOLDOWN_MS` | 21600000 | 同一合约同一事件的冷却，最少 5 分钟 |
| `ALPHAPULSE_LISTING_HOT_MS` | 7200000 | 新上架留在热池优先位的时间 |
| `ALPHAPULSE_LISTING_SNAPSHOT_CAP` | 1500 | 列表快照最多保留多少个币 |
...
3. Telegram / 飞书推送。
4. Phase 3 剩余：四小时倒计时，以及 GoPlus 安全扫描。Alpha 新上/下架已经用免费列表 diff 接上。
